...
MOST_CONNECTED_LIMIT = 5
HUB_CONNECTION_COUNT = 3
CONTEXT_ENTITY_LIMIT = 3
CHAIN_ENTITY_COUNT = 4
CHAIN_RELATIONSHIP_COUNT = 3
EXPECTED_AVERAGE_DEGREE = 1.5
...
def test_entity_degree_calculation():
    ...
    db.add_relationship(Relationship(source=person1.id, target=company.id, type="owned_by"))

    # Bob works at TechCorp (1 outgoing for Bob)
    db.add_relationship(Relationship(source=person2.id, target=company.id, type="owned_by"))
...
def test_most_connected_entities():
    ...
    db.add_relationship(Relationship(source=hub.id, target=node1.id, type="related_to"))
    db.add_relationship(Relationship(source=hub.id, target=node2.id, type="related_to"))
    db.add_relationship(Relationship(source=hub.id, target=node3.id, type="related_to"))

    # Connect node1 to node2
    db.add_relationship(Relationship(source=node1.id, target=node2.id, type="related_to"))

    # Get most connected entities
    most_connected = db.get_most_connected_entities(limit=5)

    assert len(most_connected) == MOST_CONNECTED_LIMIT
    # Hub should be first with 3 connections
    assert most_connected[0][0].name == "CentralHub"
    assert most_connected[0][1] == HUB_CONNECTION_COUNT
...
def test_entities_with_relationships_for_context():
    ...
    # Create relationships
    db.add_relationship(Relationship(source=person.id, target=company.id, type="owned_by"))
    db.add_relationship(Relationship(source=person.id, target=project.id, type="part_of"))

    # Get formatted context
    context = db.get_entities_with_relationships_for_context(limit=3, max_rels_per_entity=10)

    assert len(context) == CONTEXT_ENTITY_LIMIT
...
def test_graph_density_metrics():
    ...
    db.add_relationship(
        Relationship(source=entities[0].id, target=entities[1].id, type="related_to")
    )
    db.add_relationship(
        Relationship(source=entities[1].id, target=entities[2].id, type="related_to")
    )
    db.add_relationship(
        Relationship(source=entities[2].id, target=entities[3].id, type="related_to")
    )

    metrics = db.get_graph_density_metrics()
    assert metrics["total_entities"] == CHAIN_ENTITY_COUNT
    assert metrics["total_relationships"] == CHAIN_RELATIONSHIP_COUNT
    assert metrics["average_degree"] == EXPECTED_AVERAGE_DEGREE
    assert metrics["max_degree"] == 2  # B and C have degree 2
    assert metrics["density"] == EXPECTED_DENSITY
...
def test_context_limits():
    ...
    for i in range(1, 25):
        db.add_relationship(
            Relationship(source=entities[0].id, target=entities[i].id, type="related_to")
        )
...
def test_public_count_methods():
    ...
    assert db.get_entity_count() == LOOP_ENTITY_COUNT
