...
logging.basicConfig(  # ignore-development
    level=logging.INFO, format="%(message)s", handlers=[logging.StreamHandler(sys.stdout)]
)
logger = logging.getLogger(__name__)


# Helper function for tabulate with consistent formatting
def tabulate_with_wrapping(
    table_data: list[list[Any]], headers: list[str] | None = None, tablefmt: str = "grid"
) -> str:
    """Create a table with consistent formatting. Text wrapping is handled by the box formatting."""
    # Convert None headers to empty list to avoid type issues
    safe_headers: Sequence[str] = headers if headers is not None else []

    # Set different column widths for different types of tables
    if safe_headers and len(safe_headers) >= 4:
        # Check if this is an unused functions table
        if safe_headers and any(
            header.lower() in ["function", "type", "suggested action"] for header in safe_headers
        ):
            # Unused functions table - give more space to function name
            maxcolwidths = [6, 15, 50, 50]  # Line, Type, Function, Suggested Action
            if len(safe_headers) > 4:
                maxcolwidths.extend([30] * (len(safe_headers) - 4))
        else:
            # For tables with 4+ columns (like pyright results), give more space to message column
            maxcolwidths = [
                8,
                12,
                20,
                80,
            ]  # Line, Severity, Rule, Message (increased message width)
            if len(safe_headers) > 4:
                maxcolwidths.extend([30] * (len(safe_headers) - 4))
        return tabulate(
            table_data, headers=safe_headers, tablefmt=tablefmt, maxcolwidths=maxcolwidths
        )
    else:
        # For other tables, use standard widths with more space for content
        return tabulate(
            table_data,
...
def create_section_box(title: str, content: str = "", duration: float = 0.0) -> str:
...
class QualityAnalyzer(ast.NodeVisitor):
    ...
    def visit_If(self, node: ast.If) -> None:
        ...
        if self.is_test_file and not self._should_ignore_node(node):
            lineno = getattr(node, "lineno", 0)
            self._add_issue(node, "if_statement", f"If statement at line {lineno}")
        self.generic_visit(node)
...
def analyze_development_indicators_with_claude(
    ...
    try:
        ...
        if not z_ai_api_key:
            return False, "Z_AI_API_KEY not set for Claude analysis", []
        ...
        success, output = StreamSubprocess.run_silent(claude_cmd, cwd=Path.cwd(), env=env)
        ...
        return False, f"Error running Claude analysis: {e}", []
...
def run_claude_fix(file_path: str, issues: list[dict[str, Any]]) -> tuple[bool, str]:
    ...
    z_ai_api_key = os.environ.get("Z_AI_API_KEY")
    if not z_ai_api_key:
        return False, "Z_AI_API_KEY not set"
    ...
    claude_cmd = ["claude", "--print", "--dangerously-skip-permissions", "--", prompt]
    return StreamSubprocess.run_silent(claude_cmd, cwd=Path.cwd(), env=env)
...
def main() -> None:
    ...
    # Check for --unused-functions flag
    ...
    analysis_box = create_section_box(box_title, unused_report, analysis_elapsed)
    logger.info("\n" + analysis_box)
    ...
    dev_box = create_section_box(dev_box_title, dev_report, dev_elapsed)
    logger.info("\n" + dev_box)
            ...
            if current_analysis["total_issues"] > 0:
                ...
                if fix_success:
                    ...
                    fix_box = create_section_box(
                        "AUTOMATIC FIXES APPLIED", fix_content, fix_elapsed
                    )
                    logger.info("\n" + fix_box)
                else:
                    fix_content = "Failed to apply fixes\n\n" + fix_output
                    fix_box = create_section_box("FIX APPLICATION FAILED", fix_content, fix_elapsed)
                    logger.info("\n" + fix_box)
                    break
    ...
    pyright_box = create_section_box(
        "PYRIGHT TYPE CHECKING", formatted_type_output, type_check_elapsed
    )
    logger.info("\n" + pyright_box)
...
After fixing, the file should pass pyright type checking with no errors."""
                    ...
                    if z_ai_api_key:
                        ...
                        if claude_success:
                            ...
                            claude_box = create_section_box(
                                "CLAUDE TYPE FIXES", cleaned_output, claude_elapsed
                            )
                            logger.info("\n" + claude_box)
    ...
    ruff_box = create_section_box(
        "RUFF LINTING & FORMATTING", "\n".join(combined_output_lines), ruff_elapsed
    )
    logger.info("\n" + ruff_box)
    ...
    test_box = create_section_box("TEST EXECUTION", test_output, test_elapsed)
    logger.info("\n" + test_box)
    tests_pass = success
