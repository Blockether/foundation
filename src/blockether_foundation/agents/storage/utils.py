...
def apply_sorting(
    data: list[dict[str, Any]], sort_by: str | None = None, sort_order: str | None = None
) -> list[dict[str, Any]]:
...
def calculate_date_metrics(date_to_process: date, sessions_data: dict[str, Any]) -> dict[str, Any]:
    ...
    for session_type, sessions_count_key, runs_count_key in session_types:
        sessions = cast(list[dict[str, Any]], sessions_data.get(session_type, []) or [])
        metrics[sessions_count_key] = len(sessions)

        for session in sessions:
            if session.get("user_id"):
                all_user_ids.add(cast(str, session["user_id"]))
            session_runs = cast(list[dict[str, Any]], session.get("runs", []))
            metrics[runs_count_key] += len(session_runs)
            if runs := session_runs:
                for run in runs:
                    if model_id := run.get("model"):
                        model_provider = cast(str, run.get("model_provider", ""))
                        model_counts[f"{model_id}:{model_provider}"] = (
                            model_counts.get(f"{model_id}:{model_provider}", 0) + 1
                        )

            session_data = cast(dict[str, Any], session.get("session_data", {}))
            session_metrics = cast(dict[str, Any], session_data.get("session_metrics", {}))
            for field in token_metrics:
                token_metrics[field] += cast(int, session_metrics.get(field, 0))

    model_metrics: list[dict[str, Any]] = []
    for model, count in model_counts.items():
        model_id, model_provider = model.rsplit(":", 1)
        model_metrics.append(
            {"model_id": model_id, "model_provider": model_provider, "count": count}
        )
    ...
    return {
        ...
        "completed": date_to_process < datetime.now(UTC).date(),
...
def fetch_all_sessions_data(
    ...
    for session in sessions:
        session_date = (
            datetime.fromtimestamp(session.get("created_at", start_timestamp), tz=UTC)
            .date()
            .isoformat()
        )
...
def deserialize_cultural_knowledge_from_db(db_row: dict[str, Any]) -> CulturalKnowledge:
    ...
    content_json = cast(dict[str, Any], db_row.get("content", {}) or {})
