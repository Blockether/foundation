...
class LambdaBackgroundTaskExtension(Thread):
    ...
    This class implements the singleton pattern to ensure only one extension
    is registered with the Lambda runtime.
    """

    _instance: LambdaBackgroundTaskExtension | None = None
    _initialized: bool = False

    def __new__(cls) -> LambdaBackgroundTaskExtension:
        """Ensure only one instance of the extension exists."""
    ...
    def __init__(self) -> None:
        ...
        if LambdaBackgroundTaskExtension._initialized:
            return
        ...
        logger.info("Starting Lambda background task extension thread")  # type: ignore[unknownMemberType]
        self._register_extension()

        LambdaBackgroundTaskExtension._initialized = True
        self.start()
    ...
    def _next_event(self) -> dict[str, Any] | None:
        if not self._extension_id:
            logger.debug("No extension ID available, skipping event polling")  # type: ignore[unknownMemberType]
            return None
        ...
        try:
            with urllib_request.urlopen(req, timeout=None) as resp:
